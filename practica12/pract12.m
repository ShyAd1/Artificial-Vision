...
% --- Búsqueda de imágenes similares según clases presentes y vector de características ---
% Determinar las clases presentes en la imagen seleccionada
selectedClasses = find(classCounts(1:5) > 0);
if isempty(selectedClasses)
    disp('No se encontraron clases válidas en la imagen seleccionada; no se puede buscar similares.');
else
    % Recalcular vectores de características de los objetos de la imagen seleccionada
    selFVs = [];
    for kk = 1:numObjects
        objetoMask = (L == kk);
        [rows_obj, cols_obj] = find(objetoMask);
        if length(rows_obj) < minAreaThreshold, continue; end
        [claseIdLoc, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, fv] = extract_and_classify_features(rows_obj, cols_obj, RANGES, finalPrototypes, umbralClasificacion, claseNombres, numClases, protoSamplesAll, protoLabels, featWeights);
        if isempty(fv), continue; end
        if ismember(claseIdLoc, selectedClasses)
            selFVs = [selFVs; fv];
        end
    end
    if isempty(selFVs)
        disp('No se pudieron obtener vectores de características para los objetos seleccionados.');
    else
        selAvgFV = mean(selFVs, 1);

        % Buscar en todo el dataset imágenes que tengan EXACTAMENTE el mismo conjunto de clases
        candidateIdx = [];
        candidateFVs = [];
        candidateNames = {};
        for imgIdx = 1:numImages
            img = imageList{imgIdx};
            imgDouble2 = im2double(img);
            isObj2 = (imgDouble2(:,:,1) < 0.9) | (imgDouble2(:,:,2) < 0.9) | (imgDouble2(:,:,3) < 0.9);
            % Label
            [r2, c2] = size(isObj2);
            L2 = zeros(r2, c2);
            curLab = 0;
            for rr = 1:r2
                for cc = 1:c2
                    if isObj2(rr,cc) == 1 && L2(rr,cc) == 0
                        curLab = curLab + 1;
                        q = [rr, cc]; L2(rr,cc) = curLab;
                        while ~isempty(q)
                            p = q(1,:); q(1,:) = [];
                            for nv = 1:conectividad
                                nr = p(1) + vecinos(nv,1); nc = p(2) + vecinos(nv,2);
                                if nr>=1 && nr<=r2 && nc>=1 && nc<=c2
                                    if isObj2(nr,nc)==1 && L2(nr,nc)==0
                                        L2(nr,nc) = curLab; q(end+1,:) = [nr,nc];
                                    end
                                end
                            end
                        end
            ...
            % For each object classify and collect classes and feature vectors
            classesInImg = [];
            fvsInImg = [];
            for kk2 = 1:curLab
                maskk = (L2 == kk2);
                [rks, cks] = find(maskk);
                if length(rks) < minAreaThreshold, continue; end
                [cid, ~, ~, ~, ~, ~, ~, ~, ~, ~, ~, fv2] = extract_and_classify_features(rks, cks, RANGES, finalPrototypes, umbralClasificacion, claseNombres, numClases, protoSamplesAll, protoLabels, featWeights);
                if isempty(fv2), continue; end
                classesInImg = unique([classesInImg, cid]);
                % keep fv only if class is in selectedClasses (we compare only those)
                if ismember(cid, selectedClasses)
                    fvsInImg = [fvsInImg; fv2];
                end
            end
            % Compare class sets: must match exactly (ignoring order)
            if isequal(sort(classesInImg), sort(selectedClasses))
                if ~isempty(fvsInImg)
                    candidateIdx(end+1) = imgIdx;
                    candidateFVs = [candidateFVs; mean(fvsInImg,1)];
                    candidateNames{end+1} = fileListSorted(imgIdx).name;
                end
            end
        end

        % Compute distances and pick top-5 (exclude the same file if present)
        if isempty(candidateIdx)
            disp('No se encontraron imágenes candidatas con exactamente las mismas clases.');
        else
            distsAll = sqrt(sum((candidateFVs - repmat(selAvgFV, size(candidateFVs,1), 1)).^2, 2));
            [sortedD, sorder] = sort(distsAll);
            % Exclude the same file (if appears)
            keepMask = true(size(sorder));
            for ii = 1:length(sorder)
                candGlobalIdx = candidateIdx(sorder(ii));
                if strcmp(fileListSorted(candGlobalIdx).name, testFileName)
                    keepMask(ii) = false;
                end
            end
            sorder = sorder(keepMask);
            topK = min(5, length(sorder));
            if topK == 0
                disp('No hay otras imágenes con el mismo conjunto de clases (aparte de la seleccionada).');
            else
                disp(['Imágenes más similares (top ', num2str(topK), ') que contienen las mismas clases:']);
                for kk3 = 1:topK
                    idxSel = candidateIdx(sorder(kk3));
                    fprintf('%d) %s  (dist=%g)\n', kk3, fileListSorted(idxSel).name, sortedD(sorder(kk3)));
                end
                % Mostrar thumbnails de los topK
                figure('Name','Top similares','NumberTitle','off');
                for kk3 = 1:topK
                    idxSel = candidateIdx(sorder(kk3));
                    subplot(1, topK, kk3);
                    imshow(imageList{idxSel});
                    title([num2str(kk3) ': ' fileListSorted(idxSel).name], 'Interpreter', 'none');
                end
            end
        end
    end
...
function [claseIdentificada, claseNombreAsignado, circularity, aspectRatio, Excentricity, minR, maxR, minC, maxC, ancho, alto, featureVector] = extract_and_classify_features(rows_obj, cols_obj, RANGES, finalPrototypes, umbralClasificacion, claseNombres, numClases, protoSamplesAll, protoLabels, featWeights)
    ...
    featureVector = featureVector; % already computed and normalized above
